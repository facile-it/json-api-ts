...
const fromRelationships = (
  ...
  Object.keys(data).reduce((relationships: ResourceRecord, key: string) => {
    ...
    return merge(
      ...
      strings.length > 1
        ...
        : {
            [key]: t.array(ResourceIdentifierC).is(relationship.data)
              ...
              : fromJsonApiData(
                  resources[
                    ResourceIdentifier.iso.string.get(relationship.data)
                  ],
                  resources
                ),
          }
...
const fromJson = (u: unknown, resources: ResourceRecord): unknown =>
  JsonApiDataC.is(u) ? fromJsonApiData(u, resources) : u;
