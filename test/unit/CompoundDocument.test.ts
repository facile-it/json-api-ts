...
import { getMonad } from "fp-ts/Writer";
import "mocha";
...
describe("CompoundDocument", () => {
  describe("fromRecord", () => {
    ...
    it("should return the record itself without nested entities", () => {
      ...
      expect(f({ a: 1, b: { c: 1 }, d: [] })).to.deep.equal(
        g({ a: 1, b: { c: 1 }, d: [] })
      );
      expect(f({ _type: "a", _id: 1 })).to.deep.equal(
        g({ _type: "a", _id: 1 })
      );
    });
    ...
    it("should return a JSON:API-like record with nested entities", () => {
      ...
      expect(f({ a: { _type: "a", _id: 1 } })).to.deep.equal([
        {},
        [1, { "a:1": a1 }, { a: a1 }],
      ]);
      expect(f({ a: { b: { _type: "a", _id: 1 } } })).to.deep.equal([
        { a: {} },
        [1, { "a:1": a1 }, { "a.b": a1 }],
      ]);
      ...
      expect(
        f({ a: { _type: "a", _id: 2, b: { _type: "b", _id: 1 } } })
      ).to.deep.equal([{}, [2, { "a:2": a2, "b:1": b1 }, { a: a2 }]]);

      const a3 = { type: "a", id: "3", attributes: { b: [] } };
      expect(f({ a: { _type: "a", _id: 3, b: [] } })).to.deep.equal([
        {},
        [1, { "a:3": a3 }, { a: a3 }],
      ]);
    });
