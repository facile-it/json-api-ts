...
pipe(
  ...
  TE.map((u) => {
    const count =
      null === u || undefined === u ? 0 : ArrayC().is(u) ? u.length : 1;
  ...
  TE.map((w) => {
    ...
    const included = Object.keys(
      RelationshipsCache.lens.global.get(cache)
    ).length;
    console.log(
      ` ¦> ${included} resource(s) found among ${counter} relationship(s)`
    );
